		...
		<div class="map-contents row">
			...
			<div id="map" class="col-md-8">
				...
				<c:if test="${not empty userInfo }">
					<div>
						<a class="boarda" href="${root }/user/registboardform">글 작성</a>
					</div>
				</c:if>
						...
						<c:if test="${not empty list }">
							...
							<c:forEach items="${list }" var="item">
								<tr>
									...
									<th><a class="boarda"
										href="${root }/user/infoboard?no=${item.no }&id=${item.id }&subject=${item.subject }&contents=${item.contents }">글
											보기</a></th>
								</tr>
							</c:forEach>
						...
						<div class="col-md-9">
							<div id="boardno">
								...
								<div> <c:if
										...
										<a class="updatebtn"
											href="${root }/user/updateboardform?no=${item.no }&id=${item.id}&subject=${item.subject}&contents=${item.contents}">수정</a>
										...
										<a class="updatebtn"
											href="${root }/user/deleteboard?no=${item.no }&id=${item.id}&subject=${item.subject}&contents=${item.contents}">삭제</a>
									</c:if>
								</div>
							</div>
							<hr>
