...
<script>
	...
	/* sort 시작 */
	$(document).on("change", "#sortValue", function (){
	 ...
	 function makesortList(list) {
		 $("#aptinfolist").empty();
			for(var i=0; i<list.length;i++){

				let poll =	`
					<div class="aptNamelist">${'${list[i].aptName }'}</div>
					<div>거래 날짜 : ${'${list[i].dealYear }'}년 ${'${list[i].dealMonth }'}월
						${'${list[i].dealDay }'}일</div>
					<div>
						거래 금액 : ${'${list[i].dealAmount }'} <span class="won">(만)</span>
					</div>
					<div style="float: left;">면적 : ${'${list[i].area }'}</div>
					<button type="button" class="btn btn-warning m-3 mt-0 mb-0"
						style="float: right;" data-bs-toggle="modal"
						data-bs-target="#ModalInfo" data-bs-whatever="@mdo">주변 지하철 보기</button>
					<div>
						<br>
						<hr>
					</div>
					`;
				$("#aptinfolist").append(poll);
			}			
		}
	/* sort 끝 */
